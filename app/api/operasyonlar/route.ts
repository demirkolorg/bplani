...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Error fetching operasyonlar:", error)
    return NextResponse.json(
      { error: "Operasyonlar getirilirken bir hata oluştu" },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Error creating operasyon:", error)
    return NextResponse.json(
      { error: "Operasyon oluşturulurken bir hata oluştu" },
      { status: 500 }
    )
  }
}
