...
import { useAddTanitimArac, useRemoveTanitimArac, useTanitim } from "@/hooks/use-tanitimlar"
...
interface TanitimAracListProps {
  tanitimId: string
}

export function TanitimAracList({ tanitimId }: TanitimAracListProps) {
  const router = useRouter()
  const { t } = useLocale()
  const [deleteId, setDeleteId] = React.useState<string | null>(null)
  const [selectorOpen, setSelectorOpen] = React.useState(false)

  const { data: tanitim } = useTanitim(tanitimId)
  const { data: araclarData, isLoading: araclarLoading } = useAraclar({ limit: 100 })
  const addArac = useAddTanitimArac()
  const removeArac = useRemoveTanitimArac()

  const araclar = tanitim?.araclar || []
  ...
  return (
    <>
        ...
        <CardContent>
          {araclar.length > 0 ? (
              ...
              {araclar.map((tanitimArac) => (
                ...
                >
                  <div
                    ...
                    onClick={() => navigateToArac(tanitimArac.aracId)}
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-mono font-bold">
                          {tanitimArac.arac.plaka}
                        </span>
                        <span className="text-sm text-muted-foreground">
                          {tanitimArac.arac.model.marka.ad} {tanitimArac.arac.model.ad}
                        </span>
                        {getRenkBadge(tanitimArac.arac.renk)}
                      </div>
                      {tanitimArac.arac.kisiler && tanitimArac.arac.kisiler.length > 0 && (
                        <div className="flex items-center gap-1 text-sm text-muted-foreground mt-0.5">
                          <span className="text-xs">
                            {tanitimArac.arac.kisiler.map((ak) =>
                              `${ak.kisi.ad} ${ak.kisi.soyad}`
                            ).join(", ")}
                          </span>
                        </div>
                      )}
                      {tanitimArac.aciklama && (
                  ...
                  <Button
                    ...
                    onClick={() => setDeleteId(tanitimArac.id)}
                  >
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">{t.tanitimlar.noVehiclesYet}</p>
              <p className="text-xs">{t.tanitimlar.useAddButtonVehicle}</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <ConfirmDialog
        ...
        title={t.tanitimlar.removeVehicle}
        description={t.tanitimlar.removeVehicleConfirm}
